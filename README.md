...
### Overview
The revenue calculation logic is responsible for computing the total monthly and yearly revenue generated from book purchases by an author. This logic is essential for tracking an author's earnings and providing insights into their financial performance over time.

### Implementation Details
The revenue calculation logic involves the following steps:

1. **Fetching Purchase History**: Retrieve purchase history records for the current month and year from the PurchaseHistoryModel. This step involves querying the database for relevant purchase records based on the author's ID and the timeframe (current month or year).

2. **Calculating Total Revenue**: Calculate the total revenue generated by summing up the prices of all book purchases made by the author within the current month and year. This calculation is performed by iterating over the purchase history records and accumulating the prices of relevant purchases.

3. **Updating Author's Revenue**: Update the author's revenue records in the AuthorRevenueModel with the total revenue, current month revenue, and current year revenue. This step involves updating the author's revenue data based on the calculated totals obtained in the previous step.

4. **Sending Email Notification**: Optionally, send an email notification to the author, informing them about the revenue generated from book purchases. This step involves triggering an email notification to notify the author about their earnings.
...
async function calculateTotalRevenueForAuthor(authorId) {
  try {
    ...
    const totalRes = await PurchaseHistoryModel.aggregate([
      // Pipeline stage to calculate total value of purchases overall
      {
        $facet: {
          totalValueOverall: [
            {
